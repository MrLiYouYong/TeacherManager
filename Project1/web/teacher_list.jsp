...
<script>
    layui.use('table', function(){
        ...
        table.render({
            elem: '#test'
            ,url:'${pageContext.request.contextPath}/teacher?method=selectByPage'
            ,toolbar: '#toolbarDemo'
        ...
        table.on('tool(layFilter)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                layer.open({
                    type : 2,
                    area : ['550px', '350px'],
                    content: '${pageContext.request.contextPath}/teacher?method=getUserUpdatePage&id=' + data.id,
                });
            }
        });
            ...
            switch(obj.event){
                case 'add':
                    layer.open({
                        type : 2,
                        area : ['550px', '350px'],
                        content: '${pageContext.request.contextPath}/teacher_add.jsp'
                    });
                    break;
                case 'deleteAll':
                    ...
                    layer.confirm('真的删除行么', function(index){
                        $.post(
                            '${pageContext.request.contextPath}/teacher?method=deleteAll',
                            {'ids' : ids},
                            function(jsonObj) {
                                console.log(jsonObj);
                                if (jsonObj.code == 0) {
