...
//        System.out.println(req.getSession().getAttribute("user"));
    ...
    private void register(HttpServletRequest req, HttpServletResponse resp){
        ...
        try {
            ...
            String sql = "select id,name,password,age,address,gender from users where name=?";
            ...
            if (resultSet != null) {
                JSONUtil.toJSON(resp, JSONResult.error("用户已存在！"));
                return;
            } else if (password != password1) {
                JSONUtil.toJSON(resp, JSONResult.error("两次密码输入不一致！"));
                return;
            } else {
